...
module.exports = async (sock, chatId, message, args) => {
    ...
    try {
        ...
        console.log(`[play] loading msg sent`);
        ...
        if (ytRegex.test(query)) {
            ...
            console.log(`[play] running yts search...`);
            const { videos } = await withTimeout(yts(query), 15000, 'yts search');
            if (!videos?.length) throw new Error('No results found. Try a different name.');
            videoUrl   = videos[0].url;
            videoTitle = videos[0].title;
            console.log(`[play] yts resolved: ${videoUrl}`);
        }
        ...
        try {
            console.log(`[play] trying Madrin...`);
            result = await withTimeout(tryMadrin(videoUrl), 25000, 'Madrin');
            console.log(`[play] ✅ Madrin succeeded`);
        } catch (e) {
            console.warn(`[play] ❌ Madrin failed: ${e.message}`);
            try {
                console.log(`[play] trying Drexapp...`);
                result = await withTimeout(tryDrexapp(query), 25000, 'Drexapp');
        ...
        console.log(`[play] downloading audio bytes from resolved url...`);
